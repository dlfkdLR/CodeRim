...
**Import from Chromium** is available for DeepSeek, Factory and MiniMax. Select the browser profile directory explicitly and close the browser before importing. CodeRim reads the current localStorage manifest and records and validates exact provider origins. DeepSeek and MiniMax verify the account through the normal quota reader, reread the source, and then save an encrypted snapshot. Factory validates the WorkOS refresh response and rereads the source before saving any rotated sign-in with a version-checked write; it then requests usage. This preserves a newly rotated refresh token even when the usage request fails. It does not search obsolete token bytes or write into the browser profile. Import again after a browser logout or account replacement when a new snapshot is needed. Manual and Firefox connections remain available.

MiniMax combines localStorage with the same profile's supported plaintext Cookies schema only when independent session/group evidence agrees. Windows-protected Chromium cookies are not decrypted. Region, host, path and expiry restrictions are checked for every request. If the store is encrypted, busy, ambiguous or unsupported, use a supported Firefox or manual connection instead. Factory can retain a rotated refresh token without changing the selected account or losing the first usage result.
