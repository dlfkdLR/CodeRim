...
final class NotchCommandCodeUsageTests: XCTestCase {
    ...
    func testHeadlineIsMonthly() throws {
        let snap = ProviderSnapshot(
            ...
            fidelity: .official, status: .ok, windows: try windows(), headlineID: "monthly"
        )
    ...
    func testEmptyCreditsAreNotAReading() {
        XCTAssertThrowsError(try CommandCodeUsage.windows(
            summaryJSON: #"{"totalCost":0}"#, creditsJSON: #"{"credits":{}}"#,
            subscriptionJSON: "{}"
        )) { error in
    ...
    func testGarbageIsABadResponse() {
        XCTAssertThrowsError(try CommandCodeUsage.windows(
            summaryJSON: "not json", creditsJSON: "{}", subscriptionJSON: "{}"
        )) { error in
    ...
    func testOrgIdComesFromWhoami() {
        XCTAssertEqual(CommandCodeUsage.orgId(whoamiJSON: #"{"user":{"id":"u1"},"org":{"id":"org-9"}}"#),
                       "org-9")
    }
}

final class NotchCommandCodeCredentialsTests: XCTestCase {
    ...
    func testEnvWinsOverTheFile() throws {
        ...
        let loaded = try CommandCodeCredentials.load(
            from: url, environment: ["COMMAND_CODE_API_KEY": "user_env"]
        )
        XCTAssertEqual(loaded.apiKey, "user_env")
    }

    func testDifferentlyNamedKeyIsIgnored() throws {
        let url = try file(#"{"apiKey":"user_file"}"#)
        let loaded = try CommandCodeCredentials.load(
            from: url, environment: ["COMMANDCODE_API_KEY": "user_other"]
        )
    ...
    func testAMissingFileIsNeedsAuth() {
        XCTAssertThrowsError(try CommandCodeCredentials.load(
            from: URL(fileURLWithPath: "/tmp/missing-commandcode-\(UUID().uuidString).json"),
            environment: [:]
        )) { error in
