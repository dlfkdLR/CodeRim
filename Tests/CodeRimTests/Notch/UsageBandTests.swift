...
final class NotchUsageBandTests: XCTestCase {
    ...
    func testOnlyAmpleFollowsTheChosenAccent() {
        ...
        XCTAssertEqual(UsageBand.watch.color(accent: accent), NotchPalette.watch)
        XCTAssertEqual(UsageBand.critical.color(accent: accent), NotchPalette.critical)
        XCTAssertEqual(UsageBand.exhausted.color(accent: accent), NotchPalette.critical)
    }
}
