...
Zen workspace subscription usage.

Also known as **OpenCode Zen**. The app lists this entry as **OpenCode**.

CLI provider ID: `opencode-zen`.

## Connect

Sign in to the OpenCode workspace in your browser. Enable browser-session import for this provider, or supply its session cookie. This is the Zen entry; the Go plan has a separate provider.

1. Add **OpenCode** in **Settings → Providers → Add Provider**.
...
`CODEXBAR_OPENCODE_WORKSPACE_ID`, `OPENCODE_API_KEY`.

## Check the reading

Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider opencode-zen` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/opencode.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
