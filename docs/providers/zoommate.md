...
CLI provider ID: `zoommate`.

## Connect

Enable import of a signed-in Chrome session, or use the manual session capture described by the upstream connection guide.

1. Add **ZoomMate** in **Settings → Providers → Add Provider**.
...
Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider zoommate` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
