...
CLI provider ID: `ibmbob`.

## Connect

Enter the IBM Bob API key for the subscription team whose Bobcoin usage you want to monitor.

1. Add **IBM Bob** in **Settings → Providers → Add Provider**.
...
Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider ibmbob` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/ibm-bob.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
