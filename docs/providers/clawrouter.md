...
Router spending and monthly budgets.

CLI provider ID: `clawrouter`.

## Connect

Provide the router API key and base URL for the monthly budget and usage endpoints.

1. Add **ClawRouter** in **Settings → Providers → Add Provider**.
...
Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider clawrouter` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/clawrouter.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
