...
Credit balance and daily allowances.

CLI provider ID: `manus`.

## Connect

Import the signed-in Manus browser session or supply its session_id cookie as a manual session.

1. Add **Manus** in **Settings → Providers → Add Provider**.
...
`MANUS_COOKIE`, `MANUS_SESSION_TOKEN`.

## Check the reading

Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider manus` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/manus.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
