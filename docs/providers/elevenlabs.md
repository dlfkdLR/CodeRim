...
Subscription credits and usage.

CLI provider ID: `elevenlabs`.

## Connect

Enter an ElevenLabs API key with access to the subscription usage fields.

1. Add **ElevenLabs** in **Settings → Providers → Add Provider**.
...
`ELEVENLABS_API_KEY`, `ELEVENLABS_API_URL`, `XI_API_KEY`.

## Check the reading

Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider elevenlabs` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/elevenlabs.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
