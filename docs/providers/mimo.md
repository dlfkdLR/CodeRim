...
Account balance and token-plan usage.

CLI provider ID: `mimo`.

## Connect

Sign in to Xiaomi MiMo in a supported browser and enable browser-session import.

1. Add **Xiaomi MiMo** in **Settings → Providers → Add Provider**.
...
Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider mimo` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/mimo.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
