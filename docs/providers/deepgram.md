...
Speech and API usage metrics.

CLI provider ID: `deepgram`.

## Connect

Enter a Deepgram API key. Set DEEPGRAM_PROJECT_ID when the account has multiple projects or discovery cannot select one.

1. Add **Deepgram** in **Settings → Providers → Add Provider**.
...
`DEEPGRAM_API_KEY`, `DEEPGRAM_API_URL`, `DEEPGRAM_PROJECT_ID`.

## Check the reading

Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider deepgram` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/deepgram.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
