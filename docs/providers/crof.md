...
Credit balance and available request quotas.

CLI provider ID: `crof`.

## Connect

Enter the Crof API key for the account usage endpoint.

1. Add **Crof** in **Settings → Providers → Add Provider**.
...
`CROFAI_API_KEY`, `CROF_API_KEY`.

## Check the reading

Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider crof` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/crof.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
