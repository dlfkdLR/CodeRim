...
Team balance and API spending.

CLI provider ID: `xai`.

## Connect

Set XAI_MANAGEMENT_API_KEY and XAI_TEAM_ID for the team whose balance and spending you want to read.

1. Add **xAI** in **Settings → Providers → Add Provider**.
...
`XAI_MANAGEMENT_API_KEY`, `XAI_TEAM_ID`.

## Check the reading

Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider xai` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/xai.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
