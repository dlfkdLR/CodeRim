...
Local gateway health and route statistics.

CLI provider ID: `wayfinder`.

## Connect

Set WAYFINDER_GATEWAY_URL to the running local gateway. This entry monitors router health and statistics, not an account subscription quota.

1. Add **Wayfinder** in **Settings → Providers → Add Provider**.
...
Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider wayfinder` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/wayfinder.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
