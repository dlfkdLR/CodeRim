...
Subscription usage and quota windows.

CLI provider ID: `chutes`.

## Connect

Enter a Chutes API key for the subscription usage endpoints.

1. Add **Chutes** in **Settings → Providers → Add Provider**.
...
`CHUTES_API_KEY`, `CHUTES_API_URL`.

## Check the reading

Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider chutes` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/chutes.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
