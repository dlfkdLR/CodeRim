...
Credits and weekly limits.

CLI provider ID: `codebuff`.

## Connect

Enter a Codebuff API token or sign in to its CLI so the supported local credentials file is available.

1. Add **Codebuff** in **Settings → Providers → Add Provider**.
...
`CODEBUFF_API_KEY`, `CODEBUFF_API_URL`.

## Check the reading

Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider codebuff` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/codebuff.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
