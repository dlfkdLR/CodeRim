...
Google Cloud quota usage.

CLI provider ID: `vertexai`.

## Connect

Set up Google Cloud application-default credentials with gcloud for the relevant project. The account needs permission to read the project quota metrics.

1. Add **Vertex AI** in **Settings → Providers → Add Provider**.
...
Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider vertexai` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/vertexai.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
