...
Individual Token Plan limits.

CLI provider ID: `qwencloud`.

## Connect

Sign in to the individual Qwen Cloud Token Plan. Import the browser session or provide a manual Cookie header.

1. Add **Qwen Cloud** in **Settings → Providers → Add Provider**.
...
`QWEN_CLOUD_COOKIE`, `QWEN_CLOUD_HOST`, `QWEN_CLOUD_QUOTA_URL`.

## Check the reading

Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider qwencloud` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/qwen-cloud.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
