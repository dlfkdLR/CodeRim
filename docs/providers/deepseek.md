...
CLI provider ID: `deepseek`.

## Connect

Enter a DeepSeek API key for balance queries. A separate platform session can expose additional usage details.

1. Add **DeepSeek** in **Settings → Providers → Add Provider**.
...
`DEEPSEEK_API_KEY`, `DEEPSEEK_KEY`, `DEEPSEEK_PLATFORM_TOKEN`, `DEEPSEEK_USER_TOKEN`.

## Check the reading

Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider deepseek` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/deepseek.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
