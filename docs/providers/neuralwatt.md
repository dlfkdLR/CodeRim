...
CLI provider ID: `neuralwatt`.

## Connect

Enter a Neuralwatt API key with access to quota and usage data.

1. Add **Neuralwatt** in **Settings → Providers → Add Provider**.
...
`NEURALWATT_API_KEY`, `NEURALWATT_API_URL`.

## Check the reading

Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider neuralwatt` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/neuralwatt.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
