...
Key and team spending budgets.

CLI provider ID: `litellm`.

## Connect

Provide a LiteLLM virtual key and proxy base URL. Budget visibility depends on the permissions assigned to the key.

1. Add **LiteLLM** in **Settings → Providers → Add Provider**.
...
Hover the provider in the notch, or read the latest app snapshot with `coderim usage --provider litellm` after [installing the CLI](../cli.md). Keep CodeRim running for fresh readings.
...
Connection protocols are supplied by CodeRim's pinned CodexBar integration. [Upstream source guide](https://github.com/steipete/CodexBar/blob/51ed16bdd3abe35ec53af99818e1b5f0d2a631d3/docs/litellm.md). Configure the connection in **CodeRim**; CodexBar-specific UI or config-file instructions do not apply directly.
