...
struct CodeRimApp: App {
    ...
    init() {
        ...
        companionPublisher = CompanionSnapshotPublisher(codex: store, claude: claudeStore,
            limits: accountLimitStore, claudeIntegration: claudeIntegrationStore)
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func application(_ application: NSApplication, open urls: [URL]) {
        guard urls.contains(where: { ["coderim", "codexmeter"].contains($0.scheme ?? "") && $0.host == "usage" }) else { return }
