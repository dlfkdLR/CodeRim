...
///
/// Grok CLI signs in through `auth.x.ai` and writes the session here. CodeRim
...
///
/// Ported from the MIT-licensed Codenotch (`GrokCredentials`).
struct GrokCredentials {
    ...
    /// One signed-in CLI is the ordinary case; if several sit there, the one
