...
@MainActor
final class NotchUsageStore: ObservableObject {
    ...
    private let providers: [any NotchProvider]
    ...
    private var orderedProviders: [any NotchProvider] {
    ...
    private var queuedRefreshes: Set<String> = []
    private var refreshWaiters: [String: CheckedContinuation<Void, Never>] = [:]
    private var wakeObserver: NSObjectProtocol?
    private var appLaunchObserver: NSObjectProtocol?
    private var appTerminateObserver: NSObjectProtocol?

    init(
        providers: [any NotchProvider],
    ...
    func refreshNow() {
        guard !isRefreshing else {
            NotchLog.usage.notice("refresh skipped: one already in flight")
            return
        }
        isRefreshing = true
        lastAttempt = Date()
    ...
    private func finishRefresh(_ providerID: String) {
        ...
        refreshWaiters.removeValue(forKey: providerID)?.resume()
    }

    private func queueRefresh(_ providerID: String) {
        guard queuedRefreshes.insert(providerID).inserted else { return }
        // This task inherits the explicit refresh's interaction context, including browser opt-in.
        Task { [weak self] in
            guard let self else { return }
            await withCheckedContinuation { continuation in
                if self.refreshing.contains(providerID) { self.refreshWaiters[providerID] = continuation }
                else { continuation.resume() }
            }
            self.queuedRefreshes.remove(providerID)
    ...
    func refresh(providerID: String) {
        guard let provider = providers.first(where: { $0.id == providerID }),
              !disconnected.contains(providerID) else { return }
        if refreshing.contains(providerID) {
            queueRefresh(providerID)
            return
        }
    ...
    private static func status(for error: Error) -> ProviderStatus {
        switch error {
        case NotchProviderError.needsAuth:
            return .needsAuth
        case NotchProviderError.credentialExpired:
        ...
        case NotchProviderError.rateLimited:
            // Not an error the user can do anything about, and the last good
            // reading is still roughly true, so it reads as staleness.
            return .stale(since: Date())
        case NotchProviderError.accessDenied:
            return .accessDenied
        case NotchProviderError.nothingMetered(let why):
    ...
    private static func placeholder(_ provider: NotchProvider) -> ProviderSnapshot {
        ProviderSnapshot(
            id: provider.id,
