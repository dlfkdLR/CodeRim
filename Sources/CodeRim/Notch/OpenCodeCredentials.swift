...
/// key, and it authenticates the usage endpoint directly. Any other entry
/// (`openai`, `google`, …) is that vendor's key, and claiming one would read
/// the wrong account under OpenCode's name.
///
/// Ported from the MIT-licensed Codenotch (`OpenCodeCredentials`).
