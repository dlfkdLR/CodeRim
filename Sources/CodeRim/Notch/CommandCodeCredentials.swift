...
///
/// CodeRim never signs in — it borrows that key; refreshing is Command
/// Code's job. `COMMAND_CODE_API_KEY` wins when set, matching the desktop
/// harness; a differently named leftover is ignored on purpose.
///
/// Ported from the MIT-licensed Codenotch (`CommandCodeCredentials`).
