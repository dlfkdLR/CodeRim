...
final class NotchWindowController: NSObject, NSPopoverDelegate {
    ...
    /// Open the token-usage view — CodeRim's reason for being. From the
    /// context menu.
    ...
    func relocate(cellCount: Int? = nil) {
        ...
        // edge depends on.
        if let panel {
            NotchLog.usage.debug("panel \(NSStringFromRect(panel.frame), privacy: .public) on screen \(NSStringFromRect(screen.frame), privacy: .public)")
        }
    ...
    func peek(for duration: TimeInterval, focusing pid: pid_t?) {
        ...
        let work = DispatchWorkItem { [weak self] in
            MainActor.assumeIsolated {
                ...
                self.setPointing(false)
                self.updateInteractiveRects()
                NotchLog.usage.debug("peek folded")
            }
        }
        peekWork = work
    ...
    private func contextMenu() -> NSMenu {
        NotchLog.usage.debug("context menu opened")
        let menu = NSMenu()
