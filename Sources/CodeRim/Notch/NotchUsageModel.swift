...
struct LimitWindow: Identifiable, Codable, Equatable {
    ...
    /// Provider-rendered monetary or non-quota values; never invent an integer denominator.
    ...
    init(id: String, group: String? = nil, label: String, usedFraction: Double? = nil,
         ...
         duration: TimeInterval? = nil, displayValue: String? = nil) {
        self.id = id
        self.group = group
        self.label = label
...
struct ProviderSnapshot: Identifiable, Equatable {
    ...
    /// Tokens this provider has spent on this Mac today, when CodeRim's own
    ...
    var headlineText: String {
        ...
        if let value = headline?.displayValue { return value.count <= 10 ? value : "…" }
    ...
    var statusMessage: String? {
        ...
        case .accessDenied:
            ...
            return "CodeRim was refused access to \(displayName)'s saved "
                 + "login. Click this ring to ask again, and choose Always Allow."
        case .unsupported(let why): return why
        case .error(let why): return "Couldn't read usage — \(why)"
        case .ok: return "Connected; this account did not report usage or a quota."
        case .stale: return "Waiting for the first reading…"
        }
    }
}
