...
public sealed partial class NativeProviders
{
    ...
    private static ProviderReading ParseAlibaba(JsonElement root)
    {
        ...
        foreach (var context in contexts)
        {
            ...
            var message = (Text(context, "statusMessage") ?? Text(context, "message") ?? Text(context, "msg") ?? Text(context, "code") ?? "").ToLowerInvariant();
        ...
        var instances = contexts.Select(x => Get(x, "codingPlanInstanceInfos").ValueKind == JsonValueKind.Array ? Get(x, "codingPlanInstanceInfos") : Get(x, "coding_plan_instance_infos")).FirstOrDefault(x => x.ValueKind == JsonValueKind.Array);
