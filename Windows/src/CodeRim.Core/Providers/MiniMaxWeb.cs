...
public sealed partial class NativeProviders
{
    ...
    public static ProviderReading ParseMiniMaxWebPlan(string text, DateTimeOffset? clock = null)
    {
        ...
        ProviderReading? Structured(JsonElement root)
        {
            foreach (var context in Contexts(root))
            {
                ...
                if (models.ValueKind == JsonValueKind.Array && models.GetArrayLength() == 1 && models[0].ValueKind == JsonValueKind.Object
                    && Text(models[0], "model_name") is null)
                {
                    var single = models[0].EnumerateObject().ToDictionary(pair => pair.Name, pair => pair.Value, StringComparer.Ordinal);
                    single["model_name"] = JsonSerializer.SerializeToElement("general");
                    models = JsonSerializer.SerializeToElement(new[] { single });
                }
        ...
        var document = MiniMaxHtml(text);
        var visible = document.Visible;
        if (MiniMaxMatch(visible, @"\b(?:sign\s*in|log\s*in)\b|登录|登入").Success)
            throw new ProviderRequestException(HttpStatusCode.Unauthorized);
        if (document.Next is { } next && Structured(MiniMaxJson(next)) is { } structured) return structured;
    ...
    private async Task<ProviderReading> FetchMiniMaxWebAttemptAsync(MiniMaxWebCredential? credential, bool browserFallback, CancellationToken token)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                var metadataData = Get(metadata, "data"); if (metadataData.ValueKind != JsonValueKind.Object) metadataData = metadata;
                var currentCandidates = new[] { Get(metadataData, "current_subscribe"), Get(metadataData, "current_subscription"), Get(metadataData, "current_plan") }
                    .Where(item => item.ValueKind == JsonValueKind.Object).ToArray();
                var current = currentCandidates.Length == 1 ? currentCandidates[0] : default;
            ...
            try
            {
                for (var page = 1; page <= 10; page++)
                {
                    ...
                    if (batch.Length > 0 && !seenPages.Add(MiniMaxPageIdentity(records))) throw new InvalidDataException();
                    rows.AddRange(batch);
    ...
    private static string MiniMaxPageIdentity(JsonElement records)
    {
        static string Canonical(JsonElement value) => value.ValueKind switch {
            JsonValueKind.Object => "{" + string.Join(",", value.EnumerateObject().OrderBy(pair => pair.Name, StringComparer.Ordinal).Select(pair => JsonSerializer.Serialize(pair.Name) + ":" + Canonical(pair.Value))) + "}",
            ...
            JsonValueKind.Number when value.TryGetDecimal(out var number) => number.ToString("G29", CultureInfo.InvariantCulture),
            _ => value.GetRawText()
        };
        return Convert.ToHexString(System.Security.Cryptography.SHA256.HashData(Encoding.UTF8.GetBytes(Canonical(records))));
    }
    private static (string Visible, string? Next) MiniMaxHtml(string html)
    {
        var options = RegexOptions.IgnoreCase | RegexOptions.CultureInvariant;
        var tags = new Regex(@"<!--[\s\S]*?(?:-->|$)|<![^>]*>|<(?<closing>/)?(?<name>[a-zA-Z][a-zA-Z0-9:-]*)\b(?<attrs>(?:[^'""<>]|'[^']*'|""[^""]*"")*)>", options, MiniMaxRegexBudget);
        var attributes = new Regex(@"(?:^|\s)(?<key>[^\s=/]+)(?:\s*=\s*(?:'(?<single>[^']*)'|""(?<double>[^""]*)""|(?<bare>[^\s>]+)))?", options, MiniMaxRegexBudget);
        var visible = new StringBuilder(); string? next = null; var position = 0; var templateDepth = 0; var count = 0;
        var started = System.Diagnostics.Stopwatch.GetTimestamp();
        while (position < html.Length)
        {
            if (++count > 20000 || System.Diagnostics.Stopwatch.GetElapsedTime(started) > TimeSpan.FromSeconds(2)) throw new InvalidDataException();
            var tag = tags.Match(html, position);
            if (templateDepth == 0) visible.Append(html, position, (tag.Success ? tag.Index : html.Length) - position).Append(' ');
            if (!tag.Success) break;
            position = tag.Index + tag.Length;
            if (!tag.Groups["name"].Success) continue;
            var name = tag.Groups["name"].Value.ToLowerInvariant();
            if (name == "template") { templateDepth = Math.Max(0, templateDepth + (tag.Groups["closing"].Success ? -1 : 1)); if (templateDepth > 32) throw new InvalidDataException(); continue; }
            ...
            var close = new Regex(@"</" + name + @"\s*>", options, MiniMaxRegexBudget).Match(html, position);
            ...
            if (name == "script" && templateDepth == 0)
            {
                ...
                foreach (Match attribute in attributes.Matches(tag.Groups["attrs"].Value))
                {
                    ...
                    var value = WebUtility.HtmlDecode(attribute.Groups["single"].Success ? attribute.Groups["single"].Value
                        : attribute.Groups["double"].Success ? attribute.Groups["double"].Value : attribute.Groups["bare"].Value);
                    if (!fields.TryAdd(key, value)) throw new InvalidDataException();
                }
                if (fields.GetValueOrDefault("id") == "__NEXT_DATA__" && !fields.ContainsKey("src")
                    && (fields.GetValueOrDefault("type") is null or "" || fields["type"].Equals("application/json", StringComparison.OrdinalIgnoreCase)))
                {
                    if (next is not null) throw new InvalidDataException();
                    next = html.Substring(position, close.Index - position);
                }
            }
            position = close.Index + close.Length;
        }
        return (WebUtility.HtmlDecode(visible.ToString()), next);
    }
    private static bool MiniMaxBillingSuccess(JsonElement row)
    {
        var result = Get(row, "result");
        if (result.ValueKind is JsonValueKind.Undefined or JsonValueKind.Null || result.ValueKind == JsonValueKind.String && string.IsNullOrWhiteSpace(result.GetString()))
