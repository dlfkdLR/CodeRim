...
public sealed partial class NativeProviders : IDisposable
{
    ...
    public async Task<ProviderReading> FetchAsync(string id, string? credential, Func<string, string?> setting, Func<Uri, string?>? cookieForUri, CancellationToken token = default)
    {
        ...
        if (id == "minimax" && (setting("MINIMAX_USAGE_SOURCE")?.Equals("web", StringComparison.OrdinalIgnoreCase) == true
            || setting("MINIMAX_USAGE_SOURCE") is null or "auto" && (setting("MINIMAX_COOKIE") is not null || setting("MINIMAX_COOKIE_HEADER") is not null)))
        {
            var region = CodeRim.Core.Services.MiniMaxAuthentication.Region(setting("MINIMAX_REGION"));
            if (region is null) return new(id, ReadingState.Error, [], Message: "Choose Global or China as the MiniMax region.");
            return await FetchMiniMaxWebAsync(CodeRim.Core.Services.MiniMaxAuthentication.Parse(setting("MINIMAX_COOKIE") ?? setting("MINIMAX_COOKIE_HEADER"), region), token).ConfigureAwait(false);
        }
