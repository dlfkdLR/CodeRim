    ...
    private static async Task StepFunConnectionRegression(DashboardWindow owner, AppSettingsStore settings, CredentialVault vault, string directory)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                Require(!Descendants<Button>(window).Any(button => Equals(button.Content, "Import from Firefox…")),
                    "Manual mode offered a browser import that cannot become the active account.");
                ...
                vault.Save("setting:stepfun:STEPFUN_AUTH_MODE", "auto");
                window.Navigate("stepfun"); await Idle();
                Require(Descendants<Button>(window).Any(button => Equals(button.Content, "Import from Firefox…")),
                    "Auto mode did not offer its browser import.");
                var imported = false;
                var dialog = BrowserConnections.CreateDialog(window, "stepfun", vault, () => imported = true,
                    [new("Synthetic Firefox profile", "fixture-only")], (_, _) => Task.FromResult(jar),
                    (candidate, token) => verifier.VerifyBrowserAsync("stepfun", settings.Current, candidate, token));
                ...
                await Until(() => imported || Descendants<Button>(dialog).Single(button => Equals(button.Content, "Import sign-in")).IsEnabled, "StepFun browser import did not finish.");
                Require(imported && BrowserConnections.Load("stepfun", vault)?.Count == 1, "StepFun Firefox session was not stored in DPAPI.");
                Require((await verifier.FetchAsync("stepfun", settings.Current, CancellationToken.None)).State == ReadingState.Ready,
                    "A confirmed Firefox import did not become the active Auto connection.");
            }
