...
internal static partial class NativeSmoke
{
    private static async Task DeepSeekSourceRegression(AppSettingsStore settings, CredentialVault vault, string directory)
    {
        ...
        var keys = new[] { "DEEPSEEK_USAGE_SOURCE", "DEEPSEEK_API_KEY", "DEEPSEEK_KEY", "DEEPSEEK_PLATFORM_TOKEN", "DEEPSEEK_USER_TOKEN" };
        ...
        try
        {
            ...
            File.WriteAllText(Path.Combine(directory, "windows-deepseek-evidence.json"), JsonSerializer.Serialize(new {
                ...
                encryptedSlots = "PASS", actualConnectorStoreWpf = "PASS"
            }, JsonOptions));
        }
        finally
        {
            foreach (var key in keys) Environment.SetEnvironmentVariable(key, environment[key]);
            foreach (var key in new[] { "provider:deepseek", "provider:deepseek:web", "setting:deepseek:DEEPSEEK_USAGE_SOURCE" }) vault.Delete(key);
