...
internal static partial class NativeSmoke
{
    private static async Task ProviderAccountOwnershipRegression(AppSettingsStore settings, CredentialVault vault, string directory)
    {
        ...
        string Credential(string subject)
        {
        ...
        try
        {
            ...
            Require(store.AccountDisplay("codex") is { Label: "account-a@example.invalid", Reading.Plan: "Plan A" }, "Initial account display did not capture quota ownership.");
            GuardedFile.Replace(path, first, Credential("account-b")); row.Refresh();
            var display = store.AccountDisplay("codex");
            Require(display.Label == "account-b@example.invalid" && display.Reading is null, "External CLI switch paired account B with account A's quota.");
            var text = string.Join("|", Descendants<TextBlock>(row).Select(x => x.Text));
            Require(text.Contains("account-b@example.invalid", StringComparison.Ordinal) && !text.Contains("Plan A", StringComparison.Ordinal) && !text.Contains("85%", StringComparison.Ordinal), "Provider row rendered mixed account data.");
            ...
            File.Delete(path);
            Require(store.AccountDisplay("codex") is { Label: null, Reading: null }, "Sign-out kept private account data visible.");
